...
parse_incomplete_frame_test_() ->
    [?_test({more, _K} = erlmqtt_framing:parse(Bin)) ||
        Bin <- [<<>>,
                <<6:4, 1:1, 1:2>>,
                <<1:4, 1:1, 1:2, 1:1>>,
...
parse_incomplete_connect_test() ->
    ...
    {error, _} = erlmqtt_framing:parse(C).
...
parse_connect_test() ->
    ...
    {frame, #connect{}, _} = erlmqtt_framing:parse(C).

parse_reserved_return_code_test() ->
    C = <<32,2,0,45>>,
    {error, {reserved_return_code, 45}} = erlmqtt_framing:parse(C).

parse_oob_message_id_test() ->
    C = <<64, 2, 0, 0>>,
    {error, {out_of_bounds_message_id, 0}} = erlmqtt_framing:parse(C).

%% QoS occupies only the lowest two bits of each byte in suback
parse_bad_qoses_test() ->
    C = <<144, 3, 0,1, 100>>,
    {error, {unparsable_as_qos, _}} = erlmqtt_framing:parse(C).
...
parse_invalid_qos_test() ->
    C = <<102,2,0,1>>,
    {error, {invalid_qos_value, _}} = erlmqtt_framing:parse(C).

parse_invalid_sub_qos_test() ->
    C = <<130,8,0,1, 0,3,102,111,111,4>>,
    {error, {unparsable_as_sub, _}} = erlmqtt_framing:parse(C).
